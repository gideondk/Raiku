...
case class RaikuReactiveBucket[T: ClassTag](bucketName: String, client: RaikuClient, config: RaikuBucketConfig = RaikuBucketConfig(), prefetchSize: Int = 8,
  ...
  def fetchEnumeratee(r: RArgument = RArgument(),
    ...
    Enumeratee.mapM[String](x ⇒ (normalBucket.fetch(x, r, pr, basicQuorum, notFoundOk, IfModifiedArgument(None), onlyHead, DeletedVClockArgument(None)).start).map(_.flatMap(_.value)))
  ...
  def fetchManyEnumeratee(r: RArgument = RArgument(),
    ...
    Enumeratee.mapM[List[String]](x ⇒ (Task.sequence(x.map(normalBucket.fetch(_, r, pr, basicQuorum, notFoundOk,
  ...
  def storeEnumeratee(r: RArgument = RArgument(),
    ...
    Enumeratee.mapM[T](x ⇒ (normalBucket.store(x, r, pr, basicQuorum, notFoundOk, w, dw, returnBody,
  ...
  def storeIteratee(r: RArgument = RArgument(),
    ...
    Iteratee.foldM(0) { (result, chunk) ⇒
      normalBucket.store(chunk, r, pr, basicQuorum, notFoundOk, w, dw, ReturnBodyArgument(None), pw, IfNotModifiedArgument(None), ifNonMatched, ReturnHeadArgument(None)).start.flatMap(z ⇒
        Future(result + 1))
    }
  }
  ...
  def deleteEnumeratee(rw: RWArgument = RWArgument(),
    ...
    Enumeratee.mapM[T](x ⇒ normalBucket.delete(x, rw, VClockArgument(None), r, w, pr, pw, dw).start)
  ...
  def deleteIteratee(rw: RWArgument = RWArgument(),
    ...
    Iteratee.foldM(0)((result, chunk) ⇒ normalBucket.delete(chunk, rw, VClockArgument(None), r, w, pr, pw, dw).start.flatMap(z ⇒
      Future(result + 1)))
  }
  ...
  def binIdxEnumeratee: Enumeratee[(String, String), List[String]] =
    Enumeratee.mapM[(String, String)](x ⇒ normalBucket.fetchKeysForBinIndexByValue(x._1, x._2).start)
  ...
  def intIdxEnumeratee: Enumeratee[(String, Int), List[String]] =
    Enumeratee.mapM[(String, Int)](x ⇒ normalBucket.fetchKeysForIntIndexByValue(x._1, x._2).start)
  ...
  def rangeIdxEnumeratee: Enumeratee[(String, Range), List[String]] =
    Enumeratee.mapM[(String, Range)](x ⇒ normalBucket.fetchKeysForIntIndexByValueRange(x._1, x._2).start)
  ...
  //  def unwrapTry[A](a: Future[A]]) = a.flatMap { z ⇒
  //    z match {
