...
trait RWRequests extends Request {
  def fetchServerInfo: ValidatedFutureIO[RpbGetServerInfoResp] = {
    val req = buildRequest(request(RiakMessageType.RpbGetServerInfoReq))
  ...
  def fetchBucketProperties(bucket: String): ValidatedFutureIO[RpbBucketProps] = {
    val req = buildRequest(request(RiakMessageType.RpbGetBucketReq, RpbGetBucketReq(bucket)))
  ...
  def setBucketProperties(bucket: String, props: RpbBucketProps): ValidatedFutureIO[Unit] = {
    val req = buildRequest(request(RiakMessageType.RpbSetBucketReq, RpbSetBucketReq(bucket, props)))
    req.map(x ⇒ ())
  }
  ...
  def fetch(bucket: String, key: String, r: Option[Int] = None, pr: Option[Int] = None,
            ...
            deletedvclock: Option[Boolean] = None): ValidatedFutureIO[List[RWObject]] = {
    val req = buildRequest(request(RiakMessageType.RpbGetReq, RpbGetReq(bucket, key, r, pr, basicQuorum, notFoundOk, ifModified.map(x ⇒ x.v), head, deletedvclock)))
  ...
  def store(rwObject: RWObject, w: Option[Int] = None, dw: Option[Int] = None, returnBody: Option[Boolean] = None,
            ...
            returnHead: Option[Boolean] = None): ValidatedFutureIO[List[RWObject]] = {
    val req = buildRequest(request(RiakMessageType.RpbPutReq, RpbPutReq(rwObject.bucket, stringToByteString(rwObject.key).some,
  ...
  def deleteByKey(bucket: String, key: String, rw: Option[Int] = None, vClock: Option[VClock] = None, r: Option[Int] = None,
    ...
    val req = buildRequest(request(RiakMessageType.RpbDelReq, RpbDelReq(bucket, key, rw, vClock.map(x ⇒ x.v), r, w, pr, pw, dw)))
    req.map(x ⇒ ())
  }
  ...
  def fetchKeysForBinIndexByValue(bucket: String, idx: String, idxv: String): ValidatedFutureIO[List[String]] = {
    fetchKeysForIndexRequest(buildRequest(request(RiakMessageType.RpbIndexReq, RpbIndexReq(bucket, idx+"_bin", RpbIndexReq.IndexQueryType.valueOf(0), idxv.some.map(x ⇒ x), None, None))))
  }

  def fetchKeysForIntIndexByValue(bucket: String, idx: String, idxv: Int): ValidatedFutureIO[List[String]] = {
    fetchKeysForIndexRequest(buildRequest(request(RiakMessageType.RpbIndexReq, RpbIndexReq(bucket, idx+"_int", RpbIndexReq.IndexQueryType.valueOf(0), idxv.some.map(x ⇒ x.toString), None, None))))
  }

  def fetchKeysForIntIndexByValueRange(bucket: String, idx: String, idxr: Range): ValidatedFutureIO[List[String]] = {
    fetchKeysForIndexRequest(buildRequest(request(RiakMessageType.RpbIndexReq, RpbIndexReq(bucket, idx+"_int", RpbIndexReq.IndexQueryType.valueOf(1), None, idxr.start.some.map(x ⇒ x.toString), idxr.end.some.map(x ⇒ x.toString)))))
  }
}
